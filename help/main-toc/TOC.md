---
user-guide-title: LLM应用程序指南
breadcrumb-title: LLM应用程序
user-guide-description: 在支持的LLM平台（例如使用Adobe LLM应用程序的ChatGPT）中构建、自定义、部署和测试交互式体验。
source-git-commit: e066f66b37914e2f747176e865e26dcc074bff20
workflow-type: tm+mt
source-wordcount: '106'
ht-degree: 3%

---


# [!DNL LLM Apps] 指南 {#using}

+ [概述](/help/overview/overview.md)
+ [应用程序如何连接在一起](/help/overview/app-architecture.md)
+ 构建和启动您的第一个应用程序 {#build-first-app}
  + [自动创建您的第一个应用程序](/help/guides/create-app.md)
+ 自定义生成的应用程序 {#customize-generated-app}
  + [自定义生成的处理程序](/help/guides/customize-handler.md)
  + [自定义生成的构件](/help/guides/widgets.md)
+ 添加新操作 {#add-new-action}
  + [从头开始创建操作](/help/guides/create-action.md)
+ 连接现有EDS项目 {#connect-existing-eds}
  + [自带EDS项目](/help/guides/bring-your-own-eds.md)
+ 部署和测试 {#deploy-and-test}
  + [部署您的应用程序](/help/guides/deploy-your-app.md)
  + [将LLM应用程序作为ChatGPT插件进行测试](/help/guides/test-in-chatgpt.md)
  + [以Claude Connector形式测试LLM应用程序](/help/guides/test-in-claude.md)
+ 引用 {#reference}
  + [本地处理程序开发和测试](/help/reference/development.md)
  + [操作和构件字段](/help/reference/reference-docs.md)
  + [疑难解答](/help/reference/troubleshooting.md)
